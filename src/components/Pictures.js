...
const Picturess = (props) => {
    ...
    const deletePictures = async (id) => {
        try {
            const deleteData = await fetch(`http://157.245.184.202:8080/deletePictures/${id}`, {
                method: "DELETE"
            })
    ...
    const getPictures = async () => {
        try {
            const response = await fetch('http://157.245.184.202:8080/pictures')
        ...
        <div>
            ...
            {picturesData.length === 0 ? <h1 className="text-center mt-5 mb-5 text-white">There is not Pictures piece yet {'😌'} </h1> : (
                currentPictures.map(pictures => {
                    return (
                        <div className="card mb-5 col-sm" key={pictures.id}>
